...
const BatchUpdateModal: React.FC<Props> = ({ isOpen, onClose, id }) => {
  const { authToken } = useAuthContext();
  const [loading, setLoading] = useState(false);

  const { data: courses } = useGetCoursesQuery(authToken?.token ?? "");
  const { refetch, data: batches } = useGetBatchesQuery(authToken?.token ?? "");
  ...
  useEffect(() => {
    if (batches) {
      const found = batches?.find((row) => row._id === id);
